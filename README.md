...
| **Velocity denials from a stale program** | Structuring/compliant scenarios deny unexpectedly, or results differ run to run | Rate-limit Hooks are wall-clock only until `ISSUES.md` #7 (fix written, not yet merged upstream) lands, so a fast `--speed` replay can self-trip the gate ahead of the rule a scenario is meant to exercise. Even after #7, every replay of a kit reuses the **same fixed reference week**, so repeated runs against the same (non-`--fresh`) program can still stack transactions into the same simulated velocity window and inherit denial history from the last run. `run-stream.ts` logs the program's `seededAt` on every run — re-seed with `seed-kit.ts --kit=<id> --fresh` before any run whose results you intend to score. |
...
| **Velocity Hooks** | Rate limits window on `X-Sim-Time` when present — see `ISSUES.md` #7 for the fix (written, not yet merged upstream; previously wall-clock-only). Once merged, a high `--speed` no longer self-trips the gate within one run, but repeated runs against the same non-`--fresh` program still share the kit's fixed simulated reference week, so run history can carry over. Re-seed with `--fresh` before a run you intend to score. |
